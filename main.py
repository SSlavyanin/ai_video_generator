...
@app.post("/generate")
def generate_all(req: TopicRequest):
    cleanup_old_files()
    ...
    # 2. Генерируем аудио по фразам
    audio_paths = generate_voices(phrases)
    data["audio_paths"] = audio_paths
    save_data(session_id, data)
    
    # 3. Получаем видеоклипы по фразам
    video_paths = get_video_clips(phrases)
    data["video_paths"] = video_paths
    save_data(session_id, data)
    
    # 4. Строим итоговое видео
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = f"static/videos/final_{timestamp}.mp4"
    os.makedirs("static/videos", exist_ok=True)
